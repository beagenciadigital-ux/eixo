...
import { CONTACT_EMAIL } from '../../config/oldConfig'
...
export default function DeleteEmpire({ status, empire })
{
    ...
    return (
        <Stack spacing='sm' align='center' mt={100}>
            ...
            <Text maw={400}>{t('settings:settings.deleteEmpireText')} <a href={`mailto:${CONTACT_EMAIL}`}>{t('settings:settings.contactUs')}</a> {t('settings:settings.feedback')} </Text>
