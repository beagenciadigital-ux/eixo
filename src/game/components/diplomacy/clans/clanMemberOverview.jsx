import { Card, SimpleGrid, Grid, Text, Group, Col, Avatar } from "@mantine/core"
			...
			<Group spacing="sm" align="center" position="center" mb="sm">
				<Card sx={{ width: "380px", minHeight: "295px" }}>
					...
					<SimpleGrid cols={2} spacing={1}>
						...
						<Text align="right">
							{empire.turns.toLocaleString()} ({empire.storedturns} stored)
						</Text>
						...
						<Text align="left">{t("summary:summary.luck")}: </Text>
						<Text align="right">{luck}%</Text>
					</SimpleGrid>
				</Card>

				<Card sx={{ width: "380px", minHeight: "295px" }}>
                    <Text weight={800} size="lg">
                        {t("summary:summary.agriculture")}
                    </Text>
					<SimpleGrid cols={2} spacing={1}>
						<Text>{t("summary:summary.food")}:</Text>
						<Text align="right">{empire.food.toLocaleString()}</Text>
						<Text>
							{t("summary:summary.production")}:{" "}
							<RaceBonus value={race.mod_foodpro + era.mod_foodpro} />
						</Text>
						<Text align="right">{foodpro.toLocaleString()}</Text>
						<Text>
							{t("summary:summary.consumption")}:{" "}
							<RaceBonus value={race.mod_foodcon} />
						</Text>
						...
						<Text mt="md" weight={800} size="lg">
							{t("summary:summary.other")}
						</Text>
						<Text> </Text>
						<Text align="left">
							{t("summary:summary.explore")}:{" "}
							<RaceBonus value={era.mod_explore + race.mod_explore} />
						</Text>
                        <Text align="right">+{newLand} {t("summary:summary.acres")}</Text>
						<Text align="left">
							{t("summary:summary.blackmarket")}:{" "}
							<RaceBonus value={race.mod_market} />
						</Text>
					</SimpleGrid>
				</Card>
				...
				<Card sx={{ width: "380px", minHeight: "295px" }}>
					...
					<SimpleGrid cols={2} spacing={1}>
						...
						<Text>
							{t("summary:summary.income")}:{" "}
							<RaceBonus value={race.mod_income + era.mod_cashpro} />
						</Text>
						<Text align="right">${income.toLocaleString()}</Text>
						<Text>
							{t("summary:summary.expenses")}:{" "}
							<RaceBonus value={race.mod_expenses} />
						</Text>
						...
						<NetProduced
							title="Net"
							value={income - expenses - Math.abs(loanpayed) - corruption}
							money
						/>
				...
				<Card sx={{ width: "380px", minHeight: "295px" }}>
					<Grid columns={16}>
						...
						<Col span={5} mt={3}>
                            ...
                            <Text align="right">{empire.bldTrp.toLocaleString()}</Text>
						...
						<Col span={3} mt={3}>
							<Text align="right">
								<RaceBonus value={race.mod_buildrate} />{" "}
							</Text>
							<Text align="right">
				...
				<Card sx={{ width: "380px", minHeight: "295px" }}>
					<Grid columns={16}>
						<Col span={8}>
							...
							<Text> </Text>
							<Text mt="sm">
								{t("summary:summary.offense")}:{" "}
								<RaceBonus value={race.mod_offense} />
							</Text>
							<Text>
								{t("summary:summary.defense")}:{" "}
								<RaceBonus value={race.mod_defense} />
							</Text>
							<Text mt="sm">
								{t(`eras:eras.${eraName}.trpwiz`)}:{" "}
								<RaceBonus value={race.mod_magic} />
							</Text>
							<Text>
								{t(`eras:eras.${eraName}.runes`)}:{" "}
								<RaceBonus value={race.mod_runepro + era.mod_runepro} />
							</Text>
						</Col>
						<Col span={5} align="right" mt={3}>
							...
							<Text align="right"> </Text>
							<Text align="right" mt="sm">
								{oPower.toLocaleString()}
							</Text>
							<Text align="right">{dPower.toLocaleString()}</Text>
							<Text align="right" mt="sm">
								{empire.trpWiz.toLocaleString()}
							</Text>
							<Text align="right">{empire.runes.toLocaleString()}</Text>
						</Col>
						<Col span={3} mt={3}>
							<Text align="right">
								<RaceBonus value={race.mod_industry + era.mod_industry} />
							</Text>
				...
				<Card sx={{ width: "380px", minHeight: "295px" }}>
					...
					<Grid columns={14}>
						...
						<Col span={7}>
							<Text align="right" truncate={true}>
								{clan ? clan.clanName : "None"}
							</Text>
							<Text align="right">
								{clan ? clanRole(empire.id, clan) : t("summary:summary.none")}
							</Text>
							<Text align="right">
								{enemies === "None" ? t("summary:summary.none") : enemies}
							</Text>
							<Text align="right" mt="sm">
								{empire.offTotal} (
								{empire.offSucc
