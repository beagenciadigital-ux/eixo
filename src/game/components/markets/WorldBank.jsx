...
function WorldBankLoaded({ empire, activeGame, loadEmpire }) {
	...
	const loanRate = (
		Math.round((bankLoanRate + size) * 100) /
		100 /
		100
	).toFixed(2)
	const savingRate = (
		Math.round((bankSaveRate - size) * 100) /
		100 /
		100
	).toFixed(2)
	...
	const doBanking = async (values) => {
		try {
			const res = await Axios.post(
				`/empire/${empire.uuid}/bank?gameId=${empire.game_id}`,
				values,
			)
		...
		<main className="gnome8 vampire8 minotaur8">
			<Center mb={10}>
				<Stack spacing="sm" align="center">
					...
					<Text align="center">{t("bank.description")}</Text>
					<Text align="center">{t("bank.interest")}</Text>
					<SimpleGrid
						cols={2}
						spacing="lg"
					...
					>
						...
						>
							...
							<Group direction="row" spacing="xs" noWrap grow>
								...
								<Text align="right">
									$
									{Math.round((empire.bank * savingRate) / 52).toLocaleString()}
								</Text>
							</Group>
							<form
								onSubmit={savingsForm.onSubmit((values) => {
									doBanking(values)
								})}
							>
								<Stack align="center" spacing="sm">
									...
									/>
									<Button type="submit" disabled={roundStatus} ref={bankRef}>
										{t("bank.submit")}
									</Button>
								</Stack>
							</form>
						...
						>
							...
							<Group spacing="xs" noWrap grow>
								...
								<Text align="right">
									${Math.floor((empire.loan * loanRate) / 52).toLocaleString()}
								</Text>
							</Group>
							<form
							...
							>
								<Stack align="center" spacing="sm">
									...
									<NumberInput
										...
										disabled={loanStatus}
									/>
									<Button type="submit" disabled={roundStatus} ref={loanRef}>
										{t("bank.submit")}
									</Button>
								</Stack>
							</form>
