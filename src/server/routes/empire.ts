...
const createEmpire = async (req: Request, res: Response) => {
	...
	const empireNames = await getEmpireRepo().find({
		select: ["name"],
		where: { game_id },
	})
	...
	try {
		...
		await getEmpireEffectRepo().save(effect)
...
const getOtherEmpires = async (req: Request, res: Response) => {
	...
	const otherEmpires = await getEmpireRepo().find({
		select: [
			"empireId",
			"id",
			"name",
		...
		const newEmpires = await Promise.all(
			empires.map(async (empire) => {
				if (empire.clanId !== 0 && empire.clanId !== null) {
					const clan = await getClanRepo().find({
						select: [
							"id",
							"clanName",
							"clanPic",
...
const getEmpireEffects = async (req: Request, res: Response) => {
	...
	try {
		const effects = await getEmpireEffectRepo().find({
